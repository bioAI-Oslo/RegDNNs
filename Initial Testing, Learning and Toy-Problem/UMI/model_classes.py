...
class UMINN_Jacobi(UMINN):
    ...
    def loss_fn(self, x, y):
        ...
        loss = self.L(y_pred, y)
        for i in x:
            jacobi = functional.jacobian(
                self.L1, i.float(), create_graph=True, strict=True, vectorize=False
            )  # Jacobian of the first layer
            loss += self.lmbd * (
                torch.linalg.det(jacobi.transpose(0, 1) @ jacobi) - 1
            )  # Determinant of the Jacobian (Jacobian is a square matrix
        return loss
