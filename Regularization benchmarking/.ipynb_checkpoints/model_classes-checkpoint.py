...
class LeNet(nn.Module):
    def __init__(self, lr, momentum, in_channels=3, dropout_rate = 0.0, orthogonal = True):
    ...
    def loss_fn(
        ...
        if conf_penalty:
            conf_penalty_loss = -conf_penalty_lmbd * (y_pred * torch.log(y_pred)).sum(dim=1).mean()
    ...
    def train_step(
        ...
        if label_smoothing:
            num_classes = 10
            smoothed_labels = (1 - label_smoothing_lmbd) * labels + label_smoothing_lmbd / num_classes

        self.opt.zero_grad()

        loss, reg_loss = self.loss_fn(
            data,
            smoothed_labels.long() if label_smoothing else labels.long(),
            l1=l1,
            l1_lmbd=l1_lmbd,
            l2=l2,
            l2_lmbd=l2_lmbd,
...
class LeNet_100(nn.Module):
    def __init__(self, lr, momentum, in_channels=3, dropout_rate = 0.0, orthogonal = True):
        super(LeNet_100, self).__init__()
        self.conv1 = nn.Conv2d(
            in_channels=in_channels, out_channels=6, kernel_size=(5, 5)
        )
        self.pool = nn.MaxPool2d(kernel_size=(2, 2), stride=2)
